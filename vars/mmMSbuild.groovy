def call(String path, String arch) {
    dir (path) {
        switch (path) {
            ...
            bat (script:"msbuild cyassl.sln /t:build /p:configuration=Release64 /p:Platform=${arch}")
            break
            case ("${WORKSPACE}") : //build for "cyassl, myizip_z, microx_t"
                loadWinBat('msBuild.bat')
                sample=mm.split(', ') 
                for (int i=0; i < sample.size(); i++) {
                bat (script:"msBuild.bat ${sample[i]} ${arch}") }
            break
            case ("${WORKSPACE}/microx_t/samples") : //build loop for "microp, ucs_mm, ucs_ms, ucs_dt"
                ...
                for (int i=0; i < sample1.size(); i++) {
                    sample1[i]=='ucs_mm' ? bat(script:"msBuild_ucs_mm.bat ${sample1[i]} ${arch}") : bat(script:"msBuild.bat ${sample1[i]} ${arch}")
                    switch (sample1[i]) {
                        case ('microp') :
