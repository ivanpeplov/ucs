...
def call (Map pth=[:]) { //pth conversion - .ktr/.kjb Pentaho files
    ...
    { list = listFiles.Nix ("${pth.r}/${pth.l1}/${pth.l2}/${pth.ss}").findAll { it=~ /(?i)\.(?:ktr|kjb)$/ } } //get all .ktr/.kjb
    else { list = listFiles.Nix ("${pth.r}/${pth.l1}/${pth.l2}/${pth.ss}").findAll { it=~ /(?i)\.(?:xml)$/ } } //get all .xml
    for (i in list) {
