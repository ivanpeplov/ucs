...
def call(String path) { //v3.0 03.12.2022
    dir (path) { //path="TestSQLtoNexus"
        ...
        loadScript(place:'linux', name:'checkerSQL.sh')
        loadScript(place:'linux', name:'xsltcPTH.sh')
        lvl1 = listDir("${path}") //level 1 - group folder [MNR19]
        for (io in lvl1) { lvl2 = listDir("${path}/${io}")
            lvl2=lvl2 - 'BIN' //[AMSBatch.PTH, BonusETL_top.PTH, NTPREFS.XDB] - 'BIN'
            for (jo in lvl2) {
              ext = FilenameUtils.getExtension(jo)
              stage=listDir("${path}/${io}/${jo}") 
              if (stage != '') { for (lo in stage) { 
              pthConversion(ex:"${ext}", r:"${path}", l1:"${io}", l2:"${jo}", ss:"${lo}") } } /*substage*/
              //stage conversion
              pthConversion(ex:"${ext}", r:"${path}", l1:"${io}", l2:"${jo}") /*stage*/ 
