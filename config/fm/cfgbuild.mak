...
CFLAGS += -DSCO_UW7=1 -DIS_LITTLE_ENDIAN=1 -DNOIDENT=1
MISALIGN:=
SHARED:=-G
SOCKET:=-lsocket
THREADS_CFLAG:=-Kthread
THREADS_LFLAG:=-Kthread
DL_LFLAG:=
...
CFLAGS += -DSCO_OS5=1 -b elf -DIS_LITTLE_ENDIAN=1 -DNOIDENT=1
MISALIGN:=
SHARED:=-G
SOCKET:=-lsocket
DL_LFLAG:=
...
CFLAGS += -DIS_BIG_ENDIAN=1 -DNOIDENT=1 -Dunix=1 -G
#CFLAGS += -O -qmaxmem=4096
CFLAGS += -O -qmaxmem=8192
#LFLAGS += -brtl
LFLAGS += -brtl -berok -L/usr/lib/threads -L/usr/lib/dce -lpthreads
SOCKET:=
THREADS_LFLAG:=-lpthreads
DL_LFLAG:=
...
CFLAGS += -D'tell(x)=lseek(x,0,SEEK_CUR)' -fPIC -DFreeBSD
CFLAGS += -Dushort_t='unsigned short'
CFLAGS += -Duint_t='unsigned int'
CFLAGS += -Dulong_t='unsigned long'

CFLAGS += -DIS_LITTLE_ENDIAN -DHAS_STRSIGNAL -DNOIDENT
...
ifeq ($(ARCH),i386)
CFLAGS += -DIS_LITTLE_ENDIAN -DHAS_STRSIGNAL
else
